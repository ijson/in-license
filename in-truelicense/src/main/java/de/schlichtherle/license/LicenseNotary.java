...
import de.schlichtherle.license.exception.IllegalPasswordException;
import de.schlichtherle.license.exception.LicenseNotaryException;
...
public class LicenseNotary {
    ...
    private static final String SHA1_WITH_DSA = new ObfuscatedString(new long[]{
        0x9A584D2B8DB5BD21L, 0xFED4E11748F4A5B6L, 0x733E591B529842D1L
    }).toString();
    ...
    protected PublicKey getPublicKey() throws LicenseNotaryException,
        ...
        if (null == publicKey) {
            ...
            final KeyStore keystore = getKeyStore();
            try {
                if ((null != getKeyStoreParam().getKeyPwd())
    ...
    protected KeyStore getKeyStore() throws IOException,
        CertificateException,
        NoSuchAlgorithmException {
        if (null != keyStore) return keyStore;
        InputStream in = null;
        try {
